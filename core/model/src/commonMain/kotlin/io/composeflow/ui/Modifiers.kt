...
fun Modifier.modifierForCanvas(
    ...
    if (paletteRenderParams.isShadowNode) {
        ...
        val borderShape = if (node.trait.value is TopAppBarTrait) {
            ...
            val bottomCorner = if (project.screenHolder.showNavigation.value) {
                0.dp to 0.dp
            } else {
                16.dp to 16.dp
            }
    ...
    with(density) {
        ...
        val borderModifier =
            if (!(node.isRoot() || node.isContentRoot()) &&
                node.trait.value.isResizeable() && node.isFocused.value
            ) {
        ...
        val dragModifier = if (
            ...
            fun isPositionCloseToEdge(
                ...
                return offset.x <= threshold || offset.x >= size.width - threshold ||
                        offset.y <= threshold || offset.y >= size.height - threshold
            }
                        ...
                        onDrag = { _, dragAmount ->
                            ...
                            if (isDragging) {
                                ...
                                node.resizeHeightAsPending(
                                    ...
                                    density = density,
                                    parentHeight = node.parentNode?.boundsInWindow?.value?.height,
                                )
                            } else if (onRightDragHandler || isWidthResizing) {
                                node.resizeWidthAsPending(
                                    ...
                                    density = density,
                                    parentWidth = node.parentNode?.boundsInWindow?.value?.width,
                                )
                            }
                        },
                        onDragEnd = {
        ...
        val resizeLabelModifier = if (onBottomDragHandler || isHeightResizing) {
...
private fun Modifier.drawDropIndicator(
    node: ComposeNode,
): Modifier = composed {
    ...
    if (node.isContainer() && node.isDraggedOnBounds.value) {
        drawWithContent {
            drawContent()
            if (node.trait.value is ColumnTrait || node.trait.value is TabContentTrait ||
                        ...
                        if (node.indexToBeDropped.value >= node.children.size) {
                            ...
                            drawCircle(
                                ...
                                center =
                                    Offset(
                                        x = this.center.x,
                                        y = afterChild.boundsInWindow.value.top -
                    ...
                    0 -> {
                        ...
                        if (node.indexToBeDropped.value >= node.children.size) {
                            ...
                            drawCircle(
                                ...
                                center =
                                    Offset(
                                        x = afterChild.boundsInWindow.value.left -
...
fun Modifier.drawComposableLabel(
    ...
    if (node.isFocused.value || node.isHovered.value) {
        ...
        val textColor = if (node.isFocused.value) {
            MaterialTheme.colorScheme.onBackground.copy(alpha = 0.8f)
        } else {
            MaterialTheme.colorScheme.onSecondaryContainer.copy(alpha = 0.8f)
        }
        val labelRectColor = if (node.isFocused.value) {
...
fun Modifier.drawPadding(node: ComposeNode) = composed {
