...
/**
 * Utility objects that hold MemberNameWrapper for multiplatform KotlinPoet to avoid same MemberName are repeatedly
 * defined.
 */
object MemberHolderWrapper {
    object AndroidX {
        object Foundation {
            val Image = MemberNameWrapper.get("androidx.compose.foundation", "Image")
            val background = MemberNameWrapper.get("androidx.compose.foundation", "background", isExtension = true)
        }
        ...
        object Lazy {
            ...
            val gridItemsIndexed = MemberNameWrapper.get("androidx.compose.foundation.lazy.grid", "itemsIndexed")
        }
        ...
        object Layout {
            ...
            val align = MemberNameWrapper.get("androidx.compose.foundation.layout", "align", isExtension = true)
            ...
            val width = MemberNameWrapper.get("androidx.compose.foundation.layout", "width", isExtension = true)
        }
        ...
        object Runtime {
            ...
            val rememberSaveable = MemberNameWrapper.get("androidx.compose.runtime.saveable", "rememberSaveable")
            val rememberCoroutineScope = MemberNameWrapper.get("androidx.compose.runtime", "rememberCoroutineScope")
        }
    }

    object AppCash {
        val collectAsLazyPagingItems = MemberNameWrapper.get("app.cash.paging.compose", "collectAsLazyPagingItems")
    }

    object ComposeFlow {
        val asErrorMessage = MemberNameWrapper.get("${COMPOSEFLOW_PACKAGE}.validator", "asErrorMessage", isExtension = true)
        val isSuccess = MemberNameWrapper.get("${COMPOSEFLOW_PACKAGE}.validator", "isSuccess", isExtension = true)
        val ValidateResult = MemberNameWrapper.get("${COMPOSEFLOW_PACKAGE}.validator", "ValidateResult")
        val LocalAuthenticatedUser = MemberNameWrapper.get("${COMPOSEFLOW_PACKAGE}.auth", "LocalAuthenticatedUser")
    ...
    object Kotlin {
        object Collection {
            ...
            val toMutableList = MemberNameWrapper.get("kotlin.collections", "toMutableList", isExtension = true)
        }
    }
    ...
    object Material3 {
        ...
        val CircularProgressIndicator = MemberNameWrapper.get("androidx.compose.material3", "CircularProgressIndicator")
        ...
        val rememberDatePickerState = MemberNameWrapper.get("androidx.compose.material3", "rememberDatePickerState")
        val rememberTimePickerState = MemberNameWrapper.get("androidx.compose.material3", "rememberTimePickerState")
