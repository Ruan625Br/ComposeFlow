...
### Example 2: Navigating to a Screen When an Item is Selected
...
1. **Add a List Composable**: Place a list on your screen containing selectable items.
2. **Assign an Action**:
   - Select the list and locate the `onItemSelected` trigger in the **Inspector panel**.
...
:::note
Some actions are not implemented in the alpha version of ComposeFlow, such as media actions or
...
- **Note**: This depends on the features provided by ComposeFlow and may require additional setup.

:::warn
As of the alpha version of ComposeFlow, custom actions are not implemented, but it's on the roadmap.
:::
---
